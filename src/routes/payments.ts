...
import { PaymentRepository } from '../repositories/interfaces/PaymentRepository';
...
export function createPaymentRoutes(deps: PaymentRoutesDeps): Router {
  ...
  router.post('/settle', requireScope('process-payments'), async (req: Request, res: Response) => {
    ...
    try {
      ...
      // Hash the (message + signature) and reject if any non-failed
      // payment row in this scope already tracks it. Failed rows are
      // excluded — a verify error shouldn't permanently lock a
      // re-signed payload out.
      const authorizationHash = computeAuthorizationHash(paymentPayload.payload);
      if (await deps.payments.isAuthorizationUsed(scope, authorizationHash)) {
        res.status(409).json({
          error: X402ErrorCode.InvalidPayload,
      ...
      const persisted = await deps.payments.create(scope, {
        productId,
        amount: 0,
        amountBaseUnits,
