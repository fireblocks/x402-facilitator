      ...
      try {
        ...
        try {
          ...
          if (err instanceof InvalidStateTransitionError) {
            res.status(409).json({
              error: `Payment is in a terminal state ('${existing.status}') — cannot be force-failed.`,
            });
            return;
          }
          throw err;
  ...
  router.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        // Atomic gate: `markRefunding` requires the row to currently be
        // in `completed` or `settled`. Two concurrent /refund calls
        // both pass the read-side pre-check above (lines 159–164) but
        // only one will win this transition; the other gets
        // InvalidStateTransitionError → 409. Without this gate the
        // race would fire two Fireblocks refunds.
        try {
          await payments.markRefunding(req.scope, paymentId);
        } catch (err) {
          if (err instanceof InvalidStateTransitionError) {
            res.status(409).json({
              error: 'Refund already in progress or no longer refundable for this payment.',
            });
            return;
          }
          throw err;
        ...
        try {
          ...
          // Per-payment idempotency key on Fireblocks. A retry that
          // somehow re-enters this branch will reuse the same
          // Fireblocks tx rather than submitting a second on-chain
          // refund — matches the settle-path pattern.
          ...
          try {
            ...
            console.error('[admin/payments] markRefundFailed failed:', markErr);
          }
          const updated = await payments.get(req.scope, paymentId);
          res.status(502).json({
